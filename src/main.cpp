...
uint256 hashGenesisBlock("0x96a7919012e02d8250fe385acd3f1df44ca8271d4628430132386c1e7541f6ba");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KashmirCoin: starting difficulty is 1 / 2^12
...
{
	int64 nSubsidy = 0.001001 * COIN;
	if(nHeight < 990716) // stops generation of more coins
	{
...
static const int64 nTargetTimespan = 600; // KashmirCoin: 10 minutes
static const int64 nTargetSpacing = 101; // KashmirCoin: 101 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 1200; // KashmirCoin: 20 Minutes
static const int64 nTargetSpacingRe = 101; // KashmirCoin: 101 seconds
...
// DigiShield comes in at block 36000
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (fTestNet)
    {
        if (pindexLast->nHeight+1 >= 6) { DiffMode = 2; }
    }
    else
    {
        if (pindexLast->nHeight+1 >= 36000) { DiffMode = 2; }
    }
        
    if (DiffMode == 1) { return GetNextWorkRequired_V1(pindexLast, pblock); }
    else if (DiffMode == 2) { return DigiShield(pindexLast, pblock); }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // KashmirCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // KashmirCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x18ebaaf1b89a8cdfdb79ee0ec2d2791f719a29ccf01d1ba2d4b266a922bb8518");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1396735460;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 464196;

        if (fTestNet)
        {
            block.nTime    = 1396738142;
            block.nNonce   = 532630;
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // KashmirCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KashmirCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KashmirCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KashmirCoinMiner(CWallet *pwallet)
{
    printf("KashmirCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running KashmirCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KashmirCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KashmirCoinMiner, pwallet));
}
